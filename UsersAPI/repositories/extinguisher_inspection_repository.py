...
class ExtinguisherInspectionItemRepository:
    ...
    def get_by_id(self, item_id: int):
        return self.db.query(ExtinguisherInspectionItemDB).filter(ExtinguisherInspectionItemDB.id == item_id).first()
...
class ExtinguisherInspectionRepository:
    ...
    def get_extinguisher_for_update(self, extinguisher_id: int, tenant_id: int):
        return (
            ...
            .filter(ExtinguisherDB.id == extinguisher_id, ExtinguisherDB.tenant_id == tenant_id, ExtinguisherDB.active.is_(True))
            .with_for_update()
            .first()
        )
    ...
    def get_by_id_and_tenant(self, inspection_id: int, tenant_id: int):
        return (
            ...
            .options(joinedload(ExtinguisherInspectionDB.results).joinedload(ExtinguisherInspectionResultDB.inspection_item))
            .filter(ExtinguisherInspectionDB.id == inspection_id, ExtinguisherInspectionDB.tenant_id == tenant_id)
            .first()
        )
    ...
    def get_all_by_tenant(self, tenant_id: int, extinguisher_id: int | None = None):
        ...
        return query.order_by(ExtinguisherInspectionDB.inspection_date.desc(), ExtinguisherInspectionDB.id.desc()).all()
