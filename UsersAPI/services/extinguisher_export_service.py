...
def export_extinguishers(db: Session, current_user: UserTenantDB, tenant_id: int):
    extinguishers = ExtinguisherRepository(db).get_all_by_tenant(
        tenant_id,
    ...
    for extinguisher in extinguishers:
        ultima = (
            ...
            .filter(
                ...
                ExtinguisherInspectionDB.extinguisher_id == extinguisher.id,
            )
            .order_by(
                desc(ExtinguisherInspectionDB.inspection_date),
                desc(ExtinguisherInspectionDB.id),
            )
            .first()
        )
