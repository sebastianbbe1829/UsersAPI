...
from ..schemas.global_auth import SuperBootstrapMfaVerifyRequest, SuperBootstrapMfaVerifyResponse, SuperBootstrapRequest, SuperBootstrapResponse, SuperLoginRequest, SuperLoginResponse
from ..services.auth_audit_service import touch_active_session
...
def _create_super_token(user: GlobalUserDB, tenant: TenantDB) -> str:
    ...
    payload = {"sub": str(user.id), "name": user.email, "email": user.email, "global_user_id": user.id, "user_type": SUPER_SESSION_KIND, "session_id": user.session_id, "tenant_id": tenant.id, "tenant_slug": tenant.slug, "iat": int(now.timestamp()), "exp": int(exp)}
...
def bootstrap_super_user(datos: SuperBootstrapRequest, bootstrap_secret: str, db: Session) -> SuperBootstrapResponse:
    ...
    user = GlobalUserDB(email=email, password_hash=get_password_hash(datos.password), is_active=True, is_superuser=True, mfa_enabled=True, mfa_secret_encrypted=_encrypt_mfa_secret(secret), mfa_verified_at=None, session_id=None, created_at=now, created_by="super-bootstrap", updated_at=now, updated_by="super-bootstrap")
    db.add(user)
    db.flush()
    ...
    return SuperBootstrapResponse(id=user.id, email=user.email, mfa_enabled=True, provisioning_uri=provisioning_uri)
...
def login_super_user(datos: SuperLoginRequest, db: Session, client_ip: str | None = None) -> SuperLoginResponse:
    ...
    if user.mfa_enabled:
        if not user.mfa_verified_at:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="El MFA del usuario SUPER aún no ha sido verificado")
        if not datos.otp:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Código MFA requerido")
    ...
    logger.info("Login SUPER exitoso email=%s tenant=%s session_id=%s", user.email, tenant.slug, user.session_id)
