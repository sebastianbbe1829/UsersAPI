...
def update_extinguisher_type(type_id: int, datos: ExtinguisherTypeUpdate, db: Session):
    ...
    if item is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Tipo de extintor no encontrado")
    ...
    if "code" in cambios:
        ...
        if not code:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="El código es obligatorio")
...
def delete_extinguisher_type(type_id: int, db: Session):
    ...
    if item is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Tipo de extintor no encontrado")
    if not item.active:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="El tipo de extintor ya está inactivo")
