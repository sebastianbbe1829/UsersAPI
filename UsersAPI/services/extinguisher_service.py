...
from ..models import ExtinguisherDB, UserTenantDB
...
def create_extinguisher(
    ...
    if not code:
        raise HTTPException(
            ...
            detail="El código del extintor es obligatorio",
        )
    ...
    extinguisher = ExtinguisherDB(
        tenant_id=tenant_id,
        code=code,
        extinguisher_type=datos.extinguisher_type.strip(),
...
def list_extinguishers(
    ...
    return ExtinguisherRepository(db).get_all_by_tenant(
        tenant_id,
        include_inactive,
    )
...
def update_extinguisher(
    ...
    if "extinguisher_type" in cambios:
        cambios["extinguisher_type"] = cambios["extinguisher_type"].strip()
    ...
    try:
        ...
        db.rollback()
        logger.exception("Error de integridad actualizando extintor")
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="No fue posible actualizar el extintor",
        ) from exc
