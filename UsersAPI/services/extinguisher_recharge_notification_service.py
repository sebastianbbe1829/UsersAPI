...
NOTIFICATION_LOG_TABLE = Table(
    ...
    Column("sent_at", DateTime(timezone=True), nullable=True), schema="users_api",
)
...
class ExtinguisherRechargeNotificationService:
    ...
    def run(self, notification_date: date | None = None) -> dict:
        ...
        rows = self.db.execute(
            ...
            .where(and_(ExtinguisherDB.active.is_(True), ExtinguisherDB.next_recharge_date <= target_date, TenantDB.status == 1))
            .order_by(TenantDB.id, ExtinguisherDB.next_recharge_date.asc(), ExtinguisherDB.id.asc())
        ).all()

        by_tenant: dict[int, dict] = {}
        for extinguisher, type_name, tenant_name, tenant_slug in rows:
            bucket = by_tenant.setdefault(extinguisher.tenant_id, {"tenant_name": tenant_name, "tenant_slug": tenant_slug, "extinguishers": []})
            days_overdue = (target_date - extinguisher.next_recharge_date).days
            bucket["extinguishers"].append({
                "code": extinguisher.code, "type_name": type_name, "capacity": extinguisher.capacity,
                "location": extinguisher.location, "last_recharge_date": extinguisher.last_recharge_date,
                "next_recharge_date": extinguisher.next_recharge_date, "days_overdue": days_overdue,
        ...
        for tenant_id, data in by_tenant.items():
            ...
            if not recipients:
                logger.warning("No active ADMIN email found for tenant_id=%s; recharge notification skipped", tenant_id)
                skipped += 1
                continue
            ...
            subject = f"Alerta: {len(data['extinguishers'])} extintor(es) vencido(s) o con recarga hoy - {data['tenant_name']}"
            message = message.replace("{TOTAL}", str(len(data["extinguishers"]))).replace("{VENCIDOS}", str(overdue_count)).replace("{HOY}", str(today_count))
            ...
            for recipient in recipients:
                ...
                try:
                    send_email(recipient=recipient, subject=subject, message=message, tenant_slug=data["tenant_slug"], tenant_name=data["tenant_name"], template="default", attachments=[attachment])
                    self._mark_sent(target_date, tenant_id, recipient_key)
                    sent += 1
                except Exception:
                    self._clear_pending(target_date, tenant_id, recipient_key)
                    errors += 1
                    logger.exception("Error sending recharge notification to %s for tenant_id=%s", recipient, tenant_id)

        result = {"date": target_date.isoformat(), "tenants_with_expired_recharges": len(by_tenant), "extinguishers": len(rows), "emails_sent": sent, "tenants_without_admin_email": skipped, "email_errors": errors}
    ...
    def _build_excel_attachment(self, tenant_name: str, target_date: date, extinguishers: list[dict]) -> dict[str, str]:
        ...
        for col, header in enumerate(headers, 1):
            cell = ws.cell(4, col, header); cell.font = Font(bold=True, color=white); cell.fill = PatternFill("solid", fgColor=principal); cell.alignment = Alignment(horizontal="center", wrap_text=True); cell.border = border
        for row_idx, item in enumerate(extinguishers, 5):
            ...
            for col, value in enumerate(values, 1):
                cell = ws.cell(row_idx, col, value); cell.border = border; cell.alignment = Alignment(vertical="center", wrap_text=True)
                if row_idx % 2 == 0: cell.fill = PatternFill("solid", fgColor=gray)
            for col in (5, 6): ws.cell(row_idx, col).number_format = "dd/mm/yyyy"
        ...
        for col, width in enumerate((18, 28, 15, 30, 18, 18, 15, 15), 1): ws.column_dimensions[chr(64 + col)].width = width
    ...
    def _already_sent(self, target_date, tenant_id, recipient):
        return self.db.execute(select(NOTIFICATION_LOG_TABLE.c.status).where(and_(NOTIFICATION_LOG_TABLE.c.notification_date == target_date, NOTIFICATION_LOG_TABLE.c.tenant_id == tenant_id, NOTIFICATION_LOG_TABLE.c.recipient == recipient))).scalar_one_or_none() == "sent"

    def _mark_pending(self, target_date, tenant_id, recipient):
        statement = pg_insert(NOTIFICATION_LOG_TABLE).values(notification_date=target_date, tenant_id=tenant_id, recipient=recipient, status="pending").on_conflict_do_update(index_elements=[NOTIFICATION_LOG_TABLE.c.notification_date, NOTIFICATION_LOG_TABLE.c.tenant_id, NOTIFICATION_LOG_TABLE.c.recipient], set_={"status": "pending"}, where=NOTIFICATION_LOG_TABLE.c.status != "sent")
        self.db.execute(statement); self.db.commit()

    def _clear_pending(self, target_date, tenant_id, recipient):
        self.db.execute(NOTIFICATION_LOG_TABLE.delete().where(and_(NOTIFICATION_LOG_TABLE.c.notification_date == target_date, NOTIFICATION_LOG_TABLE.c.tenant_id == tenant_id, NOTIFICATION_LOG_TABLE.c.recipient == recipient, NOTIFICATION_LOG_TABLE.c.status == "pending"))); self.db.commit()

    def _mark_sent(self, target_date, tenant_id, recipient):
        self.db.execute(update(NOTIFICATION_LOG_TABLE).where(and_(NOTIFICATION_LOG_TABLE.c.notification_date == target_date, NOTIFICATION_LOG_TABLE.c.tenant_id == tenant_id, NOTIFICATION_LOG_TABLE.c.recipient == recipient)).values(status="sent", sent_at=func.now())); self.db.commit()

    def _get_admin_recipients(self, tenant_id):
        rows = self.db.execute(select(UserTenantDB.email).join(UserTenantRoleDB, UserTenantRoleDB.user_tenant_id == UserTenantDB.id).join(RoleDB, RoleDB.id == UserTenantRoleDB.role_id).where(and_(UserTenantDB.tenant_id == tenant_id, UserTenantDB.status == 1, RoleDB.tenant_id == tenant_id, RoleDB.code.in_(ADMIN_ROLE_CODES), RoleDB.status == 1)).distinct()).scalars().all()
