...
class CashRegisterDB(Base):
    ...
    __table_args__ = (
        ...
        Index("uq_cash_registers_open_box", "cash_box_id", unique=True, postgresql_where=text("status = 'OPEN'")),
        Index("uq_cash_registers_box_business_date", "cash_box_id", "business_date", unique=True),
        {"schema": "users_api"},
    )
