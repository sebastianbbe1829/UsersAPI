from sqlalchemy import Column, DateTime, ForeignKeyConstraint, Identity, Index, Integer, text
...
class UserCashAssignmentDB(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(["tenant_id", "user_tenant_id"], ["users_api.user_tenants.tenant_id", "users_api.user_tenants.id"]),
        ForeignKeyConstraint(["tenant_id", "branch_id"], ["users_api.branches.tenant_id", "users_api.branches.id"]),
        ForeignKeyConstraint(["tenant_id", "cash_box_id"], ["users_api.cash_boxes.tenant_id", "users_api.cash_boxes.id"]),
        Index("uq_user_cash_assignments_active_user", "tenant_id", "user_tenant_id", unique=True, postgresql_where=text("status = 1")),
