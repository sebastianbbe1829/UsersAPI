...
def pdf_report(report) -> tuple[bytes, str]:
    ...
    if generated_at.tzinfo:
        generated_at = generated_at.astimezone(COLOMBIA_TZ)
    ...
    summary = [
        [
            "Total esperado", "Total contado", "Total diferencia",
    ...
    reconciliation = [[
        "Caja", "Sucursal", "Base", "Apertura", "Hora apertura",
        "Hora cierre", "Esperado", "Contado", "Diferencia", "Resultado",
    ]]
    for row in report["registers"]:
        reconciliation.append([
            row["box"],
            row["branch"],
            f"${row['base_amount']:,.0f}",
            f"${row['opening_amount']:,.0f}",
            _fmt_dt(row["opened_at"]),
            _fmt_dt(row["closed_at"], "Pendiente"),
            f"${row['expected']:,.0f}",
            "—" if row["counted"] is None else f"${row['counted']:,.0f}",
            "—" if row["difference"] is None else f"${row['difference']:,.0f}",
            _result(row),
        ])
    story.extend([
