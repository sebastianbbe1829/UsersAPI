...
class GlobalUserDB(Base):
    ...
    is_superuser = Column(Boolean, nullable=False, server_default=text("true"))
    mfa_enabled = Column(Boolean, nullable=False, server_default=text("true"))
    mfa_secret_encrypted = Column(String(512), nullable=True)
    mfa_verified_at = Column(DateTime, nullable=True)

    session_id = Column(String(36), nullable=True, unique=True, index=True)
