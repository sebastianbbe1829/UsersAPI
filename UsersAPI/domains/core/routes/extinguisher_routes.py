...
@extinguisher_routes.get(
    "/export",
    response_description="Exportar extintores a Excel",
    summary="Exportar extintores",
...
@extinguisher_routes.get(
    ...
    summary="Buscar extintores",
    dependencies=[Depends(require_permission("EXTINGUISHER_READ"))],
)
async def buscar_extintores(
    search: str = Query("", max_length=100),
    limit: int = Query(20, ge=1, le=50),
    db: Session = Depends(get_db),
    user_tenant: UserTenantDB = Depends(get_current_tenant),
):
    return extinguisher_controller.buscar_extintores(
        db,
