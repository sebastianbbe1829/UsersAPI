...
from ..controllers.inventory_controller import export_inventory, get_inventory_item, list_inventory_items
from ..controllers.movement_controller import create_movement, export_movements, get_movement, list_movements, reverse_movement
...
@inventory_routes.get("/export", response_description="Exportar inventario a Excel", dependencies=[Depends(require_permission("INVENTORY_READ"))])
async def export_inventory_route(search: str | None = None, inventory_type_id: int | None = Query(None, gt=0), db: Session = Depends(get_db), user_tenant: UserTenantDB = Depends(get_current_tenant)):
    return export_inventory(db, cast(int, user_tenant.tenant_id), search, inventory_type_id)
...
@inventory_routes.get("/movements/export", response_description="Exportar Kardex a Excel", dependencies=[Depends(require_permission("INVENTORY_MOVEMENT_READ"))])
async def export_inventory_movements_route(product_id: int | None = Query(None, gt=0), from_date: date | None = None, to_date: date | None = None, db: Session = Depends(get_db), user_tenant: UserTenantDB = Depends(get_current_tenant)):
    return export_movements(db, cast(int, user_tenant.tenant_id), product_id, from_date, to_date)
...
async def list_inventory_movements_route(product_id: int | None = Query(None, gt=0), from_date: date | None = None, to_date: date | None = None, limit: int = Query(100, ge=1, le=500), offset: int = Query(0, ge=0), db: Session = Depends(get_db), user_tenant: UserTenantDB = Depends(get_current_tenant)):
    return list_movements(product_id, db, cast(int, user_tenant.tenant_id), limit=limit, offset=offset, from_date=from_date, to_date=to_date)
