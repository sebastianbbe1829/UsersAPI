...
def _validate_origin(data: InventoryMovementCreate, origin_type: str, reversal_of_id: UUID | None) -> None:
    expected_types = {"PURCHASE": "ENTRY", "SALE": "EXIT", "SALES_RETURN": "ENTRY", "PURCHASE_RETURN": "EXIT"}
    ...
    if expected_movement_type and data.movement_type != expected_movement_type:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"{origin_type} movements must be {expected_movement_type} movements")
    if origin_type == "SALE" and data.origin_id is None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="SALE movements require origin_id")
    if origin_type == "PURCHASE" and data.unit_purchase_price is None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="PURCHASE movements require unit_purchase_price")
    if origin_type == "REVERSAL" and reversal_of_id is None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="REVERSAL movements must reference an original movement")
    if origin_type != "REVERSAL" and reversal_of_id is not None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="reversal_of_id is only valid for REVERSAL movements")


def _calculate_weighted_average_cost(current_quantity: Decimal, current_average_cost: Decimal | None, entry_quantity: Decimal, entry_unit_cost: Decimal) -> Decimal:
...
def create_inventory_movement(data: InventoryMovementCreate, db: Session, tenant_id: int, current_user: object, reversal_of_id: UUID | None = None) -> InventoryMovementDB:
    ...
    if origin_type not in ALLOWED_ORIGIN_TYPES:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Unsupported inventory movement origin")
    ...
    if not product.active:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Inactive products cannot receive inventory movements")
    ...
    if inventory is None:
        if data.movement_type == "EXIT":
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Insufficient inventory for this movement")
        inventory = InventoryDB(tenant_id=tenant_id, product_id=data.product_id, quantity=Decimal("0"), purchase_price=None, profit_percentage=Decimal("0"), created_by=_actor_name(current_user))
    ...
    movement = InventoryMovementDB(tenant_id=tenant_id, product_id=data.product_id, movement_type=data.movement_type, origin_type=origin_type, origin_id=data.origin_id, reversal_of_id=reversal_of_id, quantity=quantity, unit_purchase_price=data.unit_purchase_price, profit_percentage=data.profit_percentage, balance_before=before, balance_after=after, notes=data.notes.strip() if data.notes else None, created_at=now, created_by=actor)
    return InventoryMovementRepository(db).add(movement)


def reverse_inventory_movement(movement_id: UUID, db: Session, tenant_id: int, current_user: object, quantity: Decimal | None = None) -> InventoryMovementDB:
    ...
    if original.origin_type == "REVERSAL" or original.reversal_of_id is not None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="A reversal movement cannot be reversed")

    already_reversed = db.query(func.coalesce(func.sum(InventoryMovementDB.quantity), 0)).filter(InventoryMovementDB.tenant_id == tenant_id, InventoryMovementDB.reversal_of_id == original.id).scalar()
    available = Decimal(original.quantity) - Decimal(already_reversed or 0)
    if available <= 0:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="This movement has already been completely reversed")
    requested = available if quantity is None else Decimal(quantity)
    if requested <= 0 or requested > available:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f"The maximum quantity available for reversal is {available}")
    ...
    if movement_type == "ENTRY" and original.unit_purchase_price is None:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="The original movement has no purchase cost to restore")
    data = InventoryMovementCreate(product_id=original.product_id, movement_type=movement_type, origin_type="REVERSAL", quantity=requested, unit_purchase_price=original.unit_purchase_price, profit_percentage=original.profit_percentage, notes=f"Reversión del movimiento {original.id}")
