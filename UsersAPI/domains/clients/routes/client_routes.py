...
from ..schemas.compliance_report import (
    ClientComplianceOverrideHistoryRead,
    ClientRestrictedListReportRead,
)
...
@client_routes.get(
    "/restricted-report",
    response_model=list[ClientRestrictedListReportRead],
    dependencies=[Depends(require_permission("CLIENT_READ"))],
)
async def restricted_lists_report_route(
    db: Session = Depends(get_db),
    user_tenant: UserTenantDB = Depends(get_current_tenant),
):
...
@client_routes.get(
    "/compliance/override-history",
    response_model=list[ClientComplianceOverrideHistoryRead],
    dependencies=[Depends(require_permission("CLIENT_READ"))],
)
async def compliance_override_history_route(
    db: Session = Depends(get_db),
    user_tenant: UserTenantDB = Depends(get_current_tenant),
):
...
@client_routes.post(
    "/{client_id}/compliance/screen",
    response_model=ClientScreeningRead,
    status_code=status.HTTP_200_OK,
    dependencies=[Depends(require_permission("CLIENT_UPDATE"))],
)
async def screen_client_route(
    client_id: UUID,
    db: Session = Depends(get_db),
    user_tenant: UserTenantDB = Depends(get_current_tenant),
):
