...
async def list_identification_types_route(
...
@catalog_routes.get(
    "/identification-types/{item_id}",
    response_model=IdentificationTypeRead,
    dependencies=[Depends(require_permission("CLIENT_READ"))],
)
async def get_identification_type_route(
    item_id: int,
    db: Session = Depends(get_db),
):
...
async def create_identification_type_route(
...
@catalog_routes.patch(
    "/identification-types/{item_id}",
    response_model=IdentificationTypeRead,
    dependencies=[Depends(require_permission("CLIENT_UPDATE"))],
)
async def update_identification_type_route(
    item_id: int,
...
@catalog_routes.delete(
    "/identification-types/{item_id}",
    status_code=status.HTTP_204_NO_CONTENT,
    dependencies=[Depends(require_permission("CLIENT_DELETE"))],
)
async def delete_identification_type_route(
    item_id: int,
    db: Session = Depends(get_db),
):
