...
class ObligationDB(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("balance >= 0 AND balance <= initial_amount", name="ck_obligations_balance"),
        CheckConstraint("status IN ('ACTIVE', 'SETTLED', 'CANCELLED')", name="ck_obligations_status"),
        {"schema": "users_api"},
    )
    ...
    sale_id = Column(UUID(as_uuid=True), ForeignKey("users_api.sales.id"), nullable=False, index=True)
