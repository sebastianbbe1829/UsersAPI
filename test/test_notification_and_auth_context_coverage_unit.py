...
def test_user_notifications_handles_email_failure_and_no_phone(monkeypatch):
    monkeypatch.setattr(
        ...
        "send_email",
        MagicMock(side_effect=RuntimeError("mail")),
    )
    whatsapp = MagicMock()
...
def test_auth_context_service_error_branches(monkeypatch):
    db = MagicMock()
    monkeypatch.setattr(
        auth_context_service.jwt,
        "decode",
        MagicMock(side_effect=ExpiredSignatureError("expired")),
    )
    ...
    monkeypatch.setattr(
        auth_context_service.jwt,
        "decode",
        MagicMock(return_value={"user_tenant_id": 8}),
    )
    with pytest.raises(HTTPException) as exc:
        auth_context_service.get_current_user_from_token("jwt", db)
...
def test_auth_context_service_user_lookup_and_tenant_mismatch(monkeypatch):
    ...
    monkeypatch.setattr(
        auth_context_service.jwt,
        "decode",
        MagicMock(return_value={"user_tenant_id": 8, "tenant_id": 7}),
    )
