...
def test_create_sale_rejects_duplicate_products_before_database_work():
    data = SaleCreate(
        items=[SaleItemCreate(product_id=1, quantity=1), SaleItemCreate(product_id=1, quantity=2)],
...
def test_create_sale_rejects_payment_total_mismatch():
    ...
    db.scalar.side_effect = [inventory(), SimpleNamespace(id=1, code="PROD-000001", name="Product", active=True)]
    data = SaleCreate(
