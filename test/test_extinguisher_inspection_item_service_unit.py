...
def test_get_inspection_item_missing(monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc:
        service.get_inspection_item(1, MagicMock())
...
def test_create_rejects_blank(datos, monkeypatch):
    ...
    monkeypatch.setattr(service, "ExtinguisherInspectionItemRepository", lambda _: repo)
    with pytest.raises(HTTPException) as exc:
        service.create_inspection_item(datos, MagicMock())
...
def test_create_duplicate(monkeypatch):
    ...
    monkeypatch.setattr(service, "ExtinguisherInspectionItemRepository", lambda _: repo)
    with pytest.raises(HTTPException) as exc:
        service.create_inspection_item(_datos(), MagicMock())
...
def test_create_integrity_error(monkeypatch):
    ...
    monkeypatch.setattr(service, "ExtinguisherInspectionItemRepository", lambda _: repo)
    with pytest.raises(HTTPException) as exc:
        service.create_inspection_item(_datos(), db)
    assert exc.value.status_code == 409
...
def test_update_success(monkeypatch):
    db = MagicMock()
    item = SimpleNamespace(id=1, code="OLD", name="Old")
    repo = MagicMock()
    repo.get_by_id.return_value = item
    repo.get_by_code.return_value = None
    monkeypatch.setattr(service, "ExtinguisherInspectionItemRepository", lambda _: repo)
    result = service.update_inspection_item(
        1, _update(code=" new ", name=" Name ", display_order=2), db
    )
...
def test_update_rejects_invalid_values(values, monkeypatch):
    ...
    monkeypatch.setattr(service, "ExtinguisherInspectionItemRepository", lambda _: repo)
    with pytest.raises(HTTPException) as exc:
        service.update_inspection_item(1, _update(**values), MagicMock())
    assert exc.value.status_code == 400
...
def test_update_duplicate_code(monkeypatch):
    ...
    repo = MagicMock()
    repo.get_by_id.return_value = item
    repo.get_by_code.return_value = SimpleNamespace(id=2)
    monkeypatch.setattr(service, "ExtinguisherInspectionItemRepository", lambda _: repo)
    with pytest.raises(HTTPException) as exc:
        service.update_inspection_item(1, _update(code="B"), MagicMock())
    assert exc.value.status_code == 409
...
def test_update_missing(monkeypatch):
    ...
    monkeypatch.setattr(service, "ExtinguisherInspectionItemRepository", lambda _: repo)
    with pytest.raises(HTTPException) as exc:
        service.update_inspection_item(1, _update(name="X"), MagicMock())
    assert exc.value.status_code == 404
...
def test_update_integrity_error(monkeypatch):
    ...
    monkeypatch.setattr(service, "ExtinguisherInspectionItemRepository", lambda _: repo)
    with pytest.raises(HTTPException) as exc:
        service.update_inspection_item(1, _update(name="B"), db)
    assert exc.value.status_code == 409
    db.rollback.assert_called_once()


def test_delete_success(monkeypatch):
    item = SimpleNamespace(id=1, active=True)
    repo = MagicMock()
    repo.get_by_id.return_value = item
    monkeypatch.setattr(service, "ExtinguisherInspectionItemRepository", lambda _: repo)
...
def test_delete_rejects_missing_or_inactive(item, monkeypatch):
    ...
    monkeypatch.setattr(service, "ExtinguisherInspectionItemRepository", lambda _: repo)
    with pytest.raises(HTTPException) as exc:
        service.delete_inspection_item(1, MagicMock())
