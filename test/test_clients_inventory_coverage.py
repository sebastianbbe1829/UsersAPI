...
from UsersAPI.domains.clients.schemas.catalog import CityCreate, DepartmentCreate
from UsersAPI.domains.clients.services import catalog_service
from UsersAPI.domains.clients.seeds import bootstrap, countries, identification_types
from UsersAPI.domains.inventory.repositories.inventory_movement_repository import (
    InventoryMovementRepository,
)
from UsersAPI.domains.inventory.services import (
    inventory_export_service,
...
def test_catalog_controller_delegates_all_operations(monkeypatch):
    ...
    for name in names:
        monkeypatch.setattr(
            catalog_controller,
            name,
            lambda *args, _s=sentinel, **kwargs: _s,
        )

    db = MagicMock()
    data = MagicMock()
    calls = [
        catalog_controller.listar_tipos_identificacion(db, True),
        catalog_controller.obtener_tipo_identificacion(db, 1),
        catalog_controller.crear_tipo_identificacion(db, data),
        catalog_controller.actualizar_tipo_identificacion(db, 1, data),
        catalog_controller.eliminar_tipo_identificacion(db, 1),
        catalog_controller.listar_paises(db, True),
        catalog_controller.obtener_pais(db, 1),
        catalog_controller.crear_pais(db, data),
        catalog_controller.actualizar_pais(db, 1, data),
        catalog_controller.eliminar_pais(db, 1),
        catalog_controller.listar_departamentos(db, 1, True),
        catalog_controller.obtener_departamento(db, 1),
        catalog_controller.crear_departamento(db, data),
        catalog_controller.actualizar_departamento(db, 1, data),
        catalog_controller.eliminar_departamento(db, 1),
        catalog_controller.listar_ciudades(db, 1, True),
        catalog_controller.obtener_ciudad(db, 1),
        catalog_controller.crear_ciudad(db, data),
        catalog_controller.actualizar_ciudad(db, 1, data),
        catalog_controller.eliminar_ciudad(db, 1),
    ]
    assert calls[:4] == [sentinel] * 4
    assert calls[5:9] == [sentinel] * 4
    assert calls[10:14] == [sentinel] * 4
    assert calls[15:19] == [sentinel] * 4
    assert calls[4] is None
    assert calls[9] is None
    assert calls[14] is None
    assert calls[19] is None
...
def test_client_controller_delegates(monkeypatch):
    ...
    monkeypatch.setattr(
        client_controller.ClientRepository,
        "get_by_id",
        lambda *args: object(),
    )
...
def test_catalog_service_not_found_and_inactive_dependencies(monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc:
        catalog_service.get_country(db, 1)
    assert exc.value.status_code == 404

    with pytest.raises(HTTPException) as exc:
        catalog_service.create_department(
            db,
            DepartmentCreate(country_id=1, code="01", name="X"),
        )
    assert exc.value.status_code == 404
    ...
    db.get.return_value = SimpleNamespace(
        id=1,
        active=False,
        country=SimpleNamespace(active=True),
    )
    with pytest.raises(HTTPException) as exc:
        catalog_service.create_city(
            db,
            CityCreate(department_id=1, code="01", name="X"),
        )
    assert exc.value.status_code == 409
...
def test_country_seed_create_and_update(monkeypatch):
    ...
    q.filter.return_value.first.side_effect = [
        None,
        SimpleNamespace(**countries.COLOMBIA),
    ]
    assert countries.seed_countries(db) == (1, 0)

    item = SimpleNamespace(**countries.COLOMBIA)
    item.name = "OLD"
...
def test_inventory_export_helpers_and_empty_exports():
    db = MagicMock()
    db.query.return_value = Query(values=[])
    response = inventory_export_service.export_inventory_excel(db, 7, "abc", 2)
    assert response.media_type == (
        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
    response = inventory_export_service.export_movements_excel(
        db,
        7,
        1,
        date(2026, 1, 1),
    ...
    with pytest.raises(ValueError):
        inventory_export_service.export_movements_excel(
            db,
            7,
            None,
            date(2026, 1, 2),
