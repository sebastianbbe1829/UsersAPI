...
def test_login_accepts_user_with_authenticate_permission(
    ...
    audit = (
        ...
        .filter(
            AuthAuditDB.user_tenant_id == user_tenant.id,
            AuthAuditDB.event_type == "LOGIN_SUCCESS",
        )
...
def test_login_locks_after_tenant_configured_attempts_and_audits_lock(
    ...
    assert correct_password.json()["detail"] == (
        "Cuenta bloqueada, comuníquese con el administrador"
    )


def test_zero_login_attempt_limit_does_not_lock_account(
    db_session: Session,
    client: TestClient,
):
    _, tenant, user_tenant, _ = create_user_context(
