...
def test_divipola_helpers_and_seed(tmp_path):
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as file:
        ...
        writer.writerow(
            {
                "Código Departamento": "5",
                "Nombre Departamento": "Antioquia",
                "Código Municipio": "5001",
...
def test_product_image_service_success_and_errors():
    ...
    response.json.return_value = {
        "photos": [
            {"id": 1, "src": {"medium": "https://img/1"}, "url": "https://p/1"},
    ...
    with patch.dict("os.environ", {"PEXELS_API_KEY": "key"}):
        with patch.object(
            service.requests,
            "get",
            side_effect=service.requests.RequestException("boom"),
        ):
            with pytest.raises(HTTPException) as exc:
                service.search_product_images("café")
...
def test_screening_bulk_sync_success_duplicate_missing_and_error():
    ...
    patches = [
        ...
        patch.object(service, "_parse_source", return_value=parsed),
        patch.object(service, "normalize_screening_text", return_value="JOHN"),
    ]
...
def test_screening_sync_job_skip_and_lock_paths():
    ...
    with patch.object(job, "ENABLED", False):
        assert job.run_restrictive_lists_sync_job() == {
            "status": "skipped",
...
def test_inventory_movement_helpers_and_list_paths():
    ...
    assert service._calculate_weighted_average_cost(
        Decimal("0"), None, Decimal("2"), Decimal("10")
    ) == Decimal("10")
    assert service._calculate_weighted_average_cost(
    ...
    with pytest.raises(HTTPException):
        service._validate_origin(
            InventoryMovementCreate(**{**base, "movement_type": "EXIT"}),
            "PURCHASE",
            None,
        )
    with pytest.raises(HTTPException):
    ...
    with pytest.raises(HTTPException):
        service.list_inventory_movements(
            MagicMock(), 1, from_date=date(2026, 2, 1), to_date=date(2026, 1, 1)
        )
...
def test_inventory_movement_create_and_reverse_paths():
    ...
    db = MagicMock()
    patches = [
        patch.object(service.ProductRepository, "get_by_id", return_value=product),
        patch.object(service.InventoryRepository, "get_by_product", return_value=inventory),
        patch.object(service.InventoryRepository, "save"),
        patch.object(service.InventoryMovementRepository, "add", return_value=movement),
    ]
    with patches[0], patches[1], patches[2], patches[3]:
...
def test_inventory_controller_and_export_paths():
    from UsersAPI.domains.inventory.controllers import movement_controller as controller
    from UsersAPI.domains.inventory.services import inventory_export_service as export_service

    movement_id = uuid4()
    db = MagicMock()
    current = SimpleNamespace()
    with patch.object(controller, "create_inventory_movement", return_value="created") as fn:
        assert controller.create_movement(MagicMock(), db, 1, current) == "created"
        fn.assert_called_once()
    with patch.object(controller, "reverse_inventory_movement", return_value="reversed"):
    ...
    response = export_service.export_inventory_excel(db, 1, search="prod", inventory_type_id=2)
    assert response.media_type.startswith("application/vnd.openxmlformats")


def test_portfolio_controller_paths():
...
def test_sales_controller_and_service_helpers():
    ...
    inventory = SimpleNamespace(
        quantity=1, purchase_price=Decimal("10"), profit_percentage=Decimal("0.1")
    )
    ...
    client = SimpleNamespace(
        status="INACTIVE", is_listed=False, compliance_status="CLEAR", id=uuid4()
    )
    ...
    sale_id = uuid4()
    with patch.object(controller, "create_sale", return_value="created"):
        assert controller.create(MagicMock(), MagicMock(), 1, MagicMock()) == "created"
    with patch.object(controller, "get_sale", return_value="sale"):
        assert controller.get(sale_id, MagicMock(), 1) == "sale"
