...
def test_bootstrap_existing_super_and_duplicate_email(monkeypatch):
    ...
    assert exc.value.status_code == 409
    assert "correo" in exc.value.detail
...
def test_verify_bootstrap_mfa_branches(monkeypatch):
    ...
    user = SimpleNamespace(mfa_verified_at=object())
    db, query = _db(user)
    with pytest.raises(HTTPException) as exc:
        service.verify_bootstrap_mfa(data, "secret", db)
    assert exc.value.status_code == 409

    user = SimpleNamespace(mfa_verified_at=None, mfa_enabled=False, mfa_secret_encrypted=None)
    db, query = _db(user)
    with pytest.raises(HTTPException) as exc:
        service.verify_bootstrap_mfa(data, "secret", db)
    assert exc.value.status_code == 409

    user = SimpleNamespace(mfa_verified_at=None, mfa_enabled=True, mfa_secret_encrypted="enc")
    db, query = _db(user)
    ...
    assert user.updated_by == "super-bootstrap-mfa"
...
def test_login_super_mfa_branches(monkeypatch):
    tenant = SimpleNamespace(id=5, slug="acme")
    user = SimpleNamespace(is_active=True, is_superuser=True, mfa_enabled=True, mfa_verified_at=None)
    data = SimpleNamespace(email="x", tenant="acme", password="p", otp=None)
    monkeypatch.setattr(service, "verify_password", lambda *_: True)
    db, query = _db(tenant, 5); query.first.side_effect = [tenant, user]
    with pytest.raises(HTTPException) as exc:
        service.login_super_user(data, db)
...
def test_get_current_super_user_session_and_tenant_validation(monkeypatch):
    ...
    db, query = _db(None, 5)
    token = jwt.encode(
        {"user_type": "SUPER", "global_user_id": 1, "session_id": "other", "tenant_id": 5, "tenant_slug": "acme"},
    ...
    token = jwt.encode(
        {"user_type": "SUPER", "global_user_id": 1, "session_id": "session", "tenant_id": 5, "tenant_slug": "acme"},
        service.settings.secret_key,
        algorithm=service.settings.algorithm,
    )
