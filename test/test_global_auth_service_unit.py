...
def test_bootstrap_existing_super_and_duplicate_email(monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc:
        service.bootstrap_super_user(
            SimpleNamespace(email="x", password="p"),
            "secret",
            db,
        )
    ...
    with pytest.raises(HTTPException) as exc:
        service.bootstrap_super_user(
            SimpleNamespace(email=" X@TEST ", password="p"),
            "secret",
            db,
        )
    assert exc.value.status_code == 409
...
def test_bootstrap_success(monkeypatch):
    ...
    result = service.bootstrap_super_user(
        SimpleNamespace(email=" X@TEST.COM ", password="p"),
        "secret",
        db,
    )
    assert result.id == 7
...
def test_verify_bootstrap_mfa_branches(monkeypatch):
    ...
    monkeypatch.setattr(
        service.pyotp.TOTP,
        "verify",
        lambda self, otp, valid_window: False,
    )
    with pytest.raises(HTTPException) as exc:
        service.verify_bootstrap_mfa(data, "secret", db)
    assert exc.value.status_code == 401
    ...
    monkeypatch.setattr(
        service.pyotp.TOTP,
        "verify",
        lambda self, otp, valid_window: True,
    )
