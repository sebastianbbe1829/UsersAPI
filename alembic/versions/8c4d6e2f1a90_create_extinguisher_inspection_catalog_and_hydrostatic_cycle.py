...
def upgrade() -> None:
    # The MVP used extinguisher_inspection_items as the detail rows of an
    # inspection. Rename that table first so the same name can become the
    # reusable inspection-item catalog.
    ...
    op.add_column(
        INSPECTIONS,
        sa.Column("inspection_number", sa.Integer(), nullable=True),
        schema=SCHEMA,
    )
    op.add_column(
        INSPECTIONS,
    ...
    seeds = [
        ("MANOMETER", "Manómetro", 1),
        ("HANDLE", "Manija", 2),
        ("PIN", "Pasador", 3),
        ("LABEL", "Etiqueta", 4),
        ("PAINT", "Pintura", 5),
        ("CYLINDER", "Cilindro", 6),
        ("SIGNAGE", "Señalización", 7),
    ]
    for code, name, display_order in seeds:
        op.execute(
            sa.text(
                f"INSERT INTO {SCHEMA}.{CATALOG} (code, name, display_order) "
                "VALUES (:code, :name, :display_order) ON CONFLICT (code) DO NOTHING"
            ).bindparams(code=code, name=name, display_order=display_order)
        )

    # Preserve any inspection detail rows created by the original MVP.
    op.add_column(
        RESULTS,
    ...
    op.create_check_constraint(
        ...
        INSPECTIONS,
        "(hydrostatic_test_performed = false AND hydrostatic_test_date IS NULL) OR "
        "(hydrostatic_test_performed = true AND hydrostatic_test_date IS NOT NULL)",
        schema=SCHEMA,
    )

    # Existing historical rows, if any, belong to the first cycle.
    op.execute(
        f"""
        UPDATE {SCHEMA}.{INSPECTIONS} i
        SET inspection_number = sub.rn,
            inspection_cycle = 1
        FROM (
            SELECT id,
                   ROW_NUMBER() OVER (PARTITION BY extinguisher_id ORDER BY inspection_date, id) AS rn
...
def downgrade() -> None:
    ...
    op.execute(
        f"ALTER INDEX {SCHEMA}.ix_users_api_extinguisher_inspection_results_inspection_id "
        "RENAME TO ix_users_api_extinguisher_inspection_items_inspection_id"
    )
    op.execute(
        f"ALTER POLICY extinguisher_inspection_results_isolation ON {SCHEMA}.{RESULTS} "
        "RENAME TO extinguisher_inspection_items_isolation"
    )
