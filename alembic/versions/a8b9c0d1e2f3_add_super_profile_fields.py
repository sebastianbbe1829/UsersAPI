...
depends_on: Union[str, Sequence[str], None] = "7b2e4f6a91c3"

SCHEMA = "users_api"


def upgrade() -> None:
    op.add_column("global_users", sa.Column("dni", sa.String(length=20), nullable=True), schema=SCHEMA)
    op.add_column("global_users", sa.Column("name", sa.String(length=100), nullable=True), schema=SCHEMA)
    op.add_column("global_users", sa.Column("phone", sa.String(length=30), nullable=True), schema=SCHEMA)
